...
if (isset($_GET['api'])) {
    ...
    $permissions_path = __DIR__ . '/../includes/permissions.php';

    // Ensure all required files exist
    if (!file_exists($db_connect_path) || !file_exists($functions_path) || !file_exists($permissions_path)) {
        echo json_encode(['status' => 'error', 'message' => 'ไม่พบไฟล์ที่จำเป็นของระบบ']);
        exit();
    }
    ...
    // Ensure permissions are loaded for API calls, similar to index.php
    if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && !isset($_SESSION['permissions'])) {
    ...
    switch ($_GET['api']) {
        ...
        case 'edit_shelter':
            ...
            if (!canManageShelter($shelter_id_to_edit)) {
        ...
        case 'save_donation':
            ...
            if (!canManageShelter($shelter_id)) {
                echo json_encode(['status' => 'error', 'message' => 'ไม่มีสิทธิ์จัดการศูนย์นี้']);
                exit();
            }
            ...
            try {
                ...
                $stmt_donation = $conn->prepare("INSERT INTO donations (shelter_id, donor_name, log_type, created_by) VALUES (?, ?, ?, ?)");
                $stmt_donation->bind_param("issi", $shelter_id, $data['donor_name'], $data['log_type'], $_SESSION['user_id']);
                if (!$stmt_donation->execute()) {
                    throw new Exception("Error saving donation record: " . $stmt_donation->error);
                }
                $donation_id = $stmt_donation->insert_id;
                $stmt_donation->close();

                // 2. Insert items into donation_items and calculate total quantity
                $total_quantity = 0;
                $stmt_items = $conn->prepare("INSERT INTO donation_items (donation_id, item_name, quantity, unit) VALUES (?, ?, ?, ?)");
                foreach ($data['items'] as $item) {
                    $quantity = intval($item['quantity']);
                    $stmt_items->bind_param("isis", $donation_id, $item['name'], $quantity, $item['unit']);
                    if (!$stmt_items->execute()) {
                        throw new Exception("Error saving donation item: " . $stmt_items->error);
                    }
                    $total_quantity += $quantity;
                }
                $stmt_items->close();

                // 3. Update shelter's current_occupancy
                $stmt_get_shelter = $conn->prepare("SELECT current_occupancy FROM shelters WHERE id = ? FOR UPDATE");
                $stmt_get_shelter->bind_param("i", $shelter_id);
                $stmt_get_shelter->execute();
                $shelter_data = $stmt_get_shelter->get_result()->fetch_assoc();
                $current_occupancy = $shelter_data['current_occupancy'];
                $stmt_get_shelter->close();

                $new_occupancy = ($data['log_type'] === 'add') 
                    ? $current_occupancy + $total_quantity 
                    : max(0, $current_occupancy - $total_quantity);

                $stmt_update_shelter = $conn->prepare("UPDATE shelters SET current_occupancy = ? WHERE id = ?");
                $stmt_update_shelter->bind_param("ii", $new_occupancy, $shelter_id);
                if (!$stmt_update_shelter->execute()) {
                    throw new Exception("Error updating shelter total: " . $stmt_update_shelter->error);
                }
                $stmt_update_shelter->close();

                // 4. Insert a summary log into shelter_logs
                $log_item_name = "บริจาคโดย " . $data['donor_name'];
                $stmt_log = $conn->prepare("INSERT INTO shelter_logs (shelter_id, item_name, item_unit, change_amount, log_type, new_total) VALUES (?, ?, ?, ?, ?, ?)");
                $item_unit_summary = "รายการ";
                $stmt_log->bind_param("issisi", $shelter_id, $log_item_name, $item_unit_summary, $total_quantity, $data['log_type'], $new_occupancy);
                if (!$stmt_log->execute()) {
                    throw new Exception("Error creating summary log: " . $stmt_log->error);
                }
                $stmt_log->close();

                $conn->commit();
                echo json_encode(['status' => 'success', 'message' => 'บันทึกข้อมูลการบริจาคสำเร็จ']);
...
<!-- START: New Donation Modal -->
<div id="donationModal" class="fixed inset-0 overflow-y-auto h-full w-full justify-center items-center z-50 hidden">
    <div class="relative mx-auto p-6 md:p-8 border-gray-200 w-full max-w-3xl shadow-2xl rounded-2xl bg-white my-8">
        <button id="closeDonationModal" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><i data-lucide="x" class="h-6 w-6"></i></button>
        <h3 id="donationModalTitle" class="text-2xl leading-6 font-bold text-gray-900 mb-6">บันทึกการบริจาค</h3>
        
        <form id="donationForm">
            <input type="hidden" id="donationShelterId" name="shelter_id">
            
            <!-- Step 1: Choose Operation -->
            <div id="donationStep1" class="space-y-6">
                <div class="bg-gray-50 p-4 rounded-lg border">
                    <h4 class="font-semibold text-gray-800 mb-3">ขั้นตอนที่ 1: เลือกประเภทรายการ</h4>
                    <div class="grid grid-cols-2 gap-3">
                        <input type="radio" name="log_type" value="add" id="donation_op_add" class="peer sr-only">
                        <label for="donation_op_add" class="flex items-center justify-center gap-2 p-4 rounded-lg border-2 border-gray-300 text-gray-800 cursor-pointer peer-checked:border-green-500 peer-checked:bg-green-50 peer-checked:text-green-700">
                            <i data-lucide="plus-circle" class="h-5 w-5"></i>
                            <span class="font-semibold">รับบริจาค (เพิ่ม)</span>
                        </label>
                        <input type="radio" name="log_type" value="subtract" id="donation_op_subtract" class="peer sr-only">
                        <label for="donation_op_subtract" class="flex items-center justify-center gap-2 p-4 rounded-lg border-2 border-gray-300 text-gray-800 cursor-pointer peer-checked:border-red-500 peer-checked:bg-red-50 peer-checked:text-red-700">
                            <i data-lucide="minus-circle" class="h-5 w-5"></i>
                            <span class="font-semibold">เบิกจ่าย (ลด)</span>
                        </label>
                    </div>
                </div>
                <div class="flex justify-end gap-4 pt-4 border-t">
                     <button type="button" class="cancel-donation-btn px-6 py-2.5 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 font-medium">ยกเลิก</button>
                     <button type="button" id="donationNextStepBtn" class="px-6 py-2.5 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 flex items-center gap-2 disabled:bg-gray-400 disabled:cursor-not-allowed" disabled>
            ...
            <!-- Step 2: Fill Details -->
            <div id="donationStep2" class="hidden space-y-6">
                <div>
                    <h4 class="font-semibold text-gray-800">ขั้นตอนที่ 2: กรอกรายละเอียด</h4>
                    <p id="donationStep2Subtitle" class="text-sm text-gray-500"></p>
                </div>
                <div>
                    <label for="donorName" class="block text-sm font-medium text-gray-700">ชื่อ-สกุล ผู้บริจาค/ผู้เบิก</label>
                    <input type="text" name="donor_name" id="donorName" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm" required>
                </div>
                <div>
                    <h5 class="text-md font-medium text-gray-800 mb-2">รายการสิ่งของ</h5>
                    <div id="donationItemsContainer" class="space-y-3">
                        <!-- Item rows will be injected here by JS -->
                    </div>
                    <button type="button" id="addDonationItemBtn" class="mt-3 inline-flex items-center gap-2 px-3 py-1.5 text-sm text-blue-700 bg-blue-100 rounded-lg hover:bg-blue-200">
                        <i data-lucide="plus" class="h-4 w-4"></i>
                        <span>เพิ่มรายการ</span>
                    </button>
                </div>
                <div class="flex justify-between gap-4 pt-4 border-t border-gray-200">
                    <button type="button" id="donationPrevStepBtn" class="px-6 py-2.5 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 font-medium flex items-center gap-2">
                        <i data-lucide="arrow-left" class="h-5 w-5"></i><span>ย้อนกลับ</span>
                    </button>
                    <button type="button" id="reviewDonationBtn" class="px-6 py-2.5 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 flex items-center gap-2">
                        <span>ตรวจสอบข้อมูล</span><i data-lucide="eye" class="h-5 w-5"></i>
                    </button>
                </div>
            </div>
        </form>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const TYPE_STYLES = {
        ...
        'ศูนย์อพยพสัตว์เลี้ยง': { icon: 'dog', color: 'yellow' }
    };
    ...
    function renderGridView(shelters) {
        ...
        dataContainer.innerHTML = shelters.map(s => {
            ...
            let actionButtons = `<button class="edit-btn text-gray-400 hover:text-blue-600" data-shelter='${JSON.stringify(s)}' title="แก้ไขข้อมูล"><i class="h-5 w-5 pointer-events-none" data-lucide="file-pen-line"></i></button>`;
            if (userRole === 'Admin') {
                actionButtons += `<button class="delete-btn text-gray-400 hover:text-red-600" data-id="${s.id}" data-name="${s.name}" title="ลบศูนย์"><i class="h-5 w-5 pointer-events-none" data-lucide="trash-2"></i></button>`;
            }

            let updateButtonHTML = '';
            if (s.type === 'ศูนย์รับบริจาค') {
                updateButtonHTML = `<button class="donation-btn flex items-center px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm font-semibold" data-shelter='${JSON.stringify(s)}'><i data-lucide="gift" class="h-4 w-4 mr-2"></i>อัปเดตยอดบริจาค</button>`;
            } else if (s.type === 'รพ.สต.' || s.type === 'ศูนย์พักพิง' || s.type === 'ศูนย์อพยพสัตว์เลี้ยง') {
                updateButtonHTML = `<button class="hospital-report-btn flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-semibold" data-shelter='${JSON.stringify(s)}'><i data-lucide="plus" class="h-4 w-4 mr-2"></i>อัปเดต</button>`;
            }


            let manageDropdownHTML = (s.type === 'รพ.สต.' || s.type === 'ศูนย์พักพิง' || s.type === 'ศูนย์อพยพสัตว์เลี้ยง') 
    ...
    function renderListView(shelters) {
        ...
        const userRole = "<?= $_SESSION['role'] ?? 'User' ?>";
        let adminActions = (s) => userRole === 'Admin' ? `<a href="#" class="delete-btn text-red-700 block px-4 py-2 text-sm hover:bg-gray-100" data-id="${s.id}" data-name="${s.name}">ลบ</a>` : '';
        ...
        dataContainer.innerHTML = `<div class="overflow-x-auto bg-white rounded-xl shadow-md"><table class="min-w-full"><thead class="bg-gray-50"><tr><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">ชื่อศูนย์</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">ผู้ประสานงาน</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">ยอดปัจจุบัน</th><th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">จัดการ</th></tr></thead><tbody class="divide-y divide-gray-200">${shelters.map(s => {
            let updateButtonHTML = '';
            if (s.type === 'ศูนย์รับบริจาค') {
                updateButtonHTML = `<button class="donation-btn flex items-center text-purple-600 hover:text-purple-900" data-shelter='${JSON.stringify(s)}'><i data-lucide="gift" class="h-4 w-4 mr-1"></i>อัปเดต</button>`;
            } else if (s.type === 'รพ.สต.' || s.type === 'ศูนย์พักพิง' || s.type === 'ศูนย์อพยพสัตว์เลี้ยง') {
                updateButtonHTML = `<button class="hospital-report-btn flex items-center text-green-600 hover:text-green-900" data-shelter='${JSON.stringify(s)}'><i data-lucide="plus" class="h-4 w-4 mr-1"></i>อัปเดต</button>`;
            }
            
            let detailsLink = (s.type === 'รพ.สต.' || s.type === 'ศูนย์พักพิง' || s.type === 'ศูนย์อพยพสัตว์เลี้ยง')
                ? `<a href="#" class="view-current-details-btn text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100" data-shelter='${JSON.stringify(s)}'>รายละเอียด</a>` : '';
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex items-center justify-end gap-4">
                        ...
                        <div class="relative inline-block text-left">
                            ...
                            <div class="manage-data-dropdown-menu hidden origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-10">
                                <div class="py-1">
                                    <a href="#" class="edit-btn text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100" data-shelter='${JSON.stringify(s)}'>แก้ไข</a>
                                    ${detailsLink}
                                    <a href="#" class="view-history-btn text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100" data-shelter='${JSON.stringify(s)}'>ประวัติ</a>
                                    ${adminActions(s)}
                                </div>
    ...
    async function openHospitalReportModal(shelter) {
        ...
        const isAnimalShelter = shelter.type === 'ศูนย์อพยพสัตว์เลี้ยง';
        
        let title = isHospital ? `เพิ่ม/ลดจำนวนผู้ป่วย` : `เพิ่ม/ลดจำนวนผู้เข้าพัก`;
        if (isAnimalShelter) {
            title = `เพิ่ม/ลดจำนวนสัตว์`;
        }

        modal.querySelector('#hospitalReportModalTitle').textContent = `${title} - ${shelter.name}`;
    ...
    // --- START: New Donation Modal Logic ---
    function openDonationModal(shelter) {
        const form = document.getElementById('donationForm');
        form.reset();
        document.getElementById('donationShelterId').value = shelter.id;
        document.getElementById('donationModalTitle').textContent = `บันทึกการบริจาค - ${shelter.name}`;
        
        // Reset to step 1
        document.getElementById('donationStep1').classList.remove('hidden');
        document.getElementById('donationStep2').classList.add('hidden');
        document.getElementById('donation_op_add').checked = false;
        document.getElementById('donation_op_subtract').checked = false;
        document.getElementById('donationNextStepBtn').disabled = true;

        // Clear and add one initial item row
        const itemsContainer = document.getElementById('donationItemsContainer');
    ...
    function addDonationItemRow() {
        const container = document.getElementById('donationItemsContainer');
        const itemIndex = container.children.length;
        const itemRow = document.createElement('div');
        itemRow.className = 'grid grid-cols-12 gap-2 items-center';
        itemRow.innerHTML = `
            <div class="col-span-5">
                <input type="text" name="items[${itemIndex}][name]" placeholder="ชื่อรายการ" class="w-full border-gray-300 rounded-lg shadow-sm text-sm" required>
            </div>
            <div class="col-span-3">
                <input type="number" name="items[${itemIndex}][quantity]" placeholder="จำนวน" min="1" class="w-full border-gray-300 rounded-lg shadow-sm text-sm" required>
            </div>
            <div class="col-span-3">
                <input type="text" name="items[${itemIndex}][unit]" placeholder="หน่วยนับ" class="w-full border-gray-300 rounded-lg shadow-sm text-sm" required>
            </div>
            <div class="col-span-1 text-right">
                <button type="button" class="remove-item-btn text-red-500 hover:text-red-700 p-1"><i data-lucide="x-circle" class="h-5 w-5 pointer-events-none"></i></button>
            </div>
        `;
    ...
    document.getElementById('donationItemsContainer').addEventListener('click', (e) => {
        if (e.target.closest('.remove-item-btn')) {
            const container = document.getElementById('donationItemsContainer');
            if (container.children.length > 1) {
                e.target.closest('.grid').remove();
            } else {
                showAlert('warning', 'ไม่สามารถลบได้', 'ต้องมีอย่างน้อย 1 รายการ');
            }
        }
    });

    document.getElementById('addDonationItemBtn').addEventListener('click', addDonationItemRow);

    document.querySelectorAll('input[name="log_type"]').forEach(radio => {
        radio.addEventListener('change', () => {
            document.getElementById('donationNextStepBtn').disabled = false;
        });
    });

    document.getElementById('donationNextStepBtn').addEventListener('click', () => {
        const logType = document.querySelector('input[name="log_type"]:checked').value;
        const subtitle = document.getElementById('donationStep2Subtitle');
        subtitle.textContent = logType === 'add' ? 'กรอกรายละเอียดสิ่งของที่รับบริจาค' : 'กรอกรายละเอียดสิ่งของที่เบิกจ่าย';
        document.getElementById('donationStep1').classList.add('hidden');
        document.getElementById('donationStep2').classList.remove('hidden');
    });

    document.getElementById('donationPrevStepBtn').addEventListener('click', () => {
        document.getElementById('donationStep2').classList.add('hidden');
        document.getElementById('donationStep1').classList.remove('hidden');
    });

    document.querySelectorAll('.cancel-donation-btn').forEach(btn => {
        btn.addEventListener('click', () => donationModal.classList.add('hidden'));
    });
    document.getElementById('closeDonationModal').addEventListener('click', () => donationModal.classList.add('hidden'));

    document.getElementById('reviewDonationBtn').addEventListener('click', () => {
        const form = document.getElementById('donationForm');
        ...
        const logType = formData.get('log_type') === 'add' ? 'รับบริจาค (เพิ่ม)' : 'เบิกจ่าย (ลด)';
        const donorName = formData.get('donor_name');
        
        let itemsHtml = '<ul class="list-disc list-inside text-left">';
        let totalQuantity = 0;
        const itemRows = document.querySelectorAll('#donationItemsContainer > div');
        itemRows.forEach((row, index) => {
            ...
            const unit = row.querySelector(`input[name="items[${index}][unit]"]`).value;
            itemsHtml += `<li>${name}: ${quantity} ${unit}</li>`;
            totalQuantity += parseInt(quantity) || 0;
        });
        itemsHtml += '</ul>';

        Swal.fire({
            title: 'ยืนยันข้อมูลการบริจาค',
            html: `
                <div class="text-left space-y-2 p-4 bg-gray-50 rounded-lg">
                    <p><strong>ประเภท:</strong> ${logType}</p>
                    <p><strong>ชื่อผู้บริจาค/ผู้เบิก:</strong> ${donorName}</p>
                    <p><strong>รวมทั้งหมด:</strong> ${totalQuantity} ชิ้น</p>
                    <hr class="my-2">
    ...
    dataContainer.addEventListener('click', e => {
        ...
        const donationBtn = e.target.closest('.donation-btn');
        ...
        if (donationBtn) { const shelterData = JSON.parse(donationBtn.dataset.shelter); openDonationModal(shelterData); }
