        ...
        case 'approve_user':
            ...
            if ($stmt->execute()) {
                if ($stmt->affected_rows > 0) {
                    ...
                    echo json_encode(['status' => 'error', 'message' => 'ไม่พบผู้ใช้ที่รออนุมัติ หรืออาจเกิดข้อผิดพลาด']);
                }
            } else {
                echo json_encode(['status' => 'error', 'message' => 'อนุมัติผู้ใช้งานไม่สำเร็จ: ' . $stmt->error]);
            }
            $stmt->close();
            break;
        default:
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function openUserModal(user = null) {
        ...
        if (user) {
            ...
            // Don't set the password value in the form for security
            const { password, ...userData } = user;
            Object.keys(userData).forEach(key => {
                const input = userForm.elements[key];
                if (input) {
                    if (key === 'assigned_shelter_id') {
                        assignedShelterSelect.value = userData[key] || '';
            ...
            userForm.elements.password.value = ''; // Ensure password field is empty
    ...
    userTableBody.addEventListener('click', e => {
        ...
        if (e.target.classList.contains('approve-btn')) {
            ...
            const name = e.target.dataset.name;
            Swal.fire({
                title: 'ยืนยันการอนุมัติ?', text: `คุณต้องการอนุมัติผู้ใช้ "${name}" หรือไม่?`, icon: 'question',
                showCancelButton: true, confirmButtonColor: '#28a745', cancelButtonColor: '#6b7280',
                confirmButtonText: 'ใช่, อนุมัติ!', cancelButtonText: 'ยกเลิก'
            }).then(async (result) => {
                if (result.isConfirmed) {
                    const res = await submitForm(`${API_URL}?api=approve_user`, { id });
                    showAlert(res.success ? 'success' : 'error', res.message);
                }
            });
        }
